...
That portable foundation now exists. `presentation/DirtyRegionTracker` owns the characterized legacy invalidation, clipping, split, overflow-to-full, and reset rules and is used by the Windows refresh path. `presentation/PixelSurface` provides tested indexed8/RGB565 heap storage, aligned pitch, palettes, color keys, clipped/overlap-safe copying, fill, and nearest-neighbor stretch. Its golden framebuffer test covers padded pitch, clipping, both color-key directions, cursor composition, and backup/restore direction. `presentation/Presenter` describes only final framebuffer upload, suspend, and resume without exposing SDL or Windows types. Explicit system-memory `SGPVSurface` objects and the reserved frame buffer now use `PixelSurface` as canonical storage for engine locks and software rendering. A lazy DirectDraw mirror preserves mixed blits, Windows WinFont, tactical scrolling, and final refresh during the transition; default/video-memory generic surfaces and the reserved primary/back/cursor surfaces remain DirectDraw-owned.
...
5. In progress: the application loop, cursor/input backend, exactly-once shutdown, dirty-region tracker, presenter contract, and heap-backed indexed/RGB565 surface primitives are portable and tested. Explicit system-memory generic surfaces and the reserved frame buffer are now PixelSurface-canonical, with a lazy DirectDraw mirror retaining current Wine/cnc-ddraw behavior for mixed copies, WinFont, tactical scrolling, and refresh. The remaining presentation work is to convert default/video-memory generic surfaces, move back-buffer and cursor composition onto portable storage, and leave DirectDraw only as the final Windows presenter before replacing it with SDL.
