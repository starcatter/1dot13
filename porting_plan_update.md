...
5. In progress: the application loop, cursor/input backend, exactly-once shutdown, dirty-region tracker, presenter contract, and heap-backed indexed/RGB565 surface primitives are portable and tested. Explicit system-memory generic surfaces are now PixelSurface-canonical, with a lazy DirectDraw mirror retaining current Wine/cnc-ddraw behavior for mixed copies and WinFont. The remaining presentation work is to convert default/video-memory and reserved logical surfaces, move cursor composition onto portable storage, and leave DirectDraw only as the final Windows presenter before replacing it with SDL.
