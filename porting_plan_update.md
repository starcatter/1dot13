...
The project-owned `ja2::text` service now converts between UTF-8 byte strings and the engine's fixed-width `CHAR16` UTF-16 strings. It offers strict conversion with project-owned errors and explicit replacement conversion for compatibility boundaries. Its bounded-buffer adapter records validity and truncation, always terminates a non-empty destination, and never splits a UTF-16 surrogate pair. Characterization tests cover multilingual text, supplementary characters, embedded NULs, malformed UTF-8, lone UTF-16 surrogates, fixed-buffer truncation, and third-party exception containment. utf8cpp was upgraded from 2.3.4 to the pinned 4.1.1 release and remains private to the adapter.

Lua wide strings, equipment-template filenames, and the localized Utils and Laptop XML loaders are the first migrated consumers. Lua conversions now preserve explicit Lua string lengths, and the Lua wide-string helpers no longer include Windows or depend on `wcslen`, `wcscpy`, or `wcscat`. The migrated XML loaders now have deterministic replacement and safe fixed-buffer truncation instead of direct Win32 calls; two Laptop loaders retain their historical explicit capacity limits even though their backing arrays have one additional element. Approximately 94 direct `MultiByteToWideChar`/`WideCharToMultiByte` call lines remain; the remaining localized XML groups, clipboard/input, multiplayer, and gameplay boundaries should be migrated in characterized slices rather than by blind replacement.
