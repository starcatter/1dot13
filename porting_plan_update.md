...
Lua wide strings, equipment-template filenames, and the localized item/language/sender XML loaders are the first migrated consumers. Lua conversions now preserve explicit Lua string lengths, and the Lua wide-string helpers no longer include Windows or depend on `wcslen`, `wcscpy`, or `wcscat`. The migrated XML loaders now have deterministic replacement and safe fixed-buffer truncation instead of direct Win32 calls. Approximately 111 direct `MultiByteToWideChar`/`WideCharToMultiByte` call lines remain; the other localized XML groups, clipboard/input, multiplayer, and gameplay boundaries should be migrated in characterized slices rather than by blind replacement.
