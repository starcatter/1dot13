...
The first audio extraction slice is now implemented. `soundman.h` and the legacy sound-manager policy no longer expose FMOD declarations; device initialization, file callback adaptation, stream creation, channel playback, looping, volume, pan, and shutdown go through `Audio::Backend`. Windows selects an FMOD/DirectSound adapter with one-to-one legacy behavior, while the Linux shared core selects a deterministic null backend. The complete legacy `soundman.cpp` policy translation unit now passes the strict native GCC build, and its real initialize/query/shutdown lifecycle is linked and executed by the native audio test. Linux initialization deliberately reports no audio device, leaving normal no-sound behavior in place until a real mixer exists. Sound diagnostics now use the portable writable log store rather than the bfVFS logger. FMOD calls and calling conventions are confined to `sgp/audio/windows/FmodAudioBackend.cpp`. The null backend is a compile/test foothold, not the final portable mixer. Stracciatella independently validates miniaudio as a viable decoder/converter foundation, but its SDL callback, ring-buffer service thread, and channel state machine show that adopting it is a real backend implementation rather than a mechanical API rename; 1.13's existing callback and timing policy should remain above that implementation.
...
- `platform/windows/ApplicationHost.cpp` is the only implementation of the current `MsgWaitForMultipleObjectsEx` plus one-message dispatch policy; an SDL host can implement the same contract.
...
DirectDraw has been removed from the engine-facing `sgp/video.h`; native handles and object accessors now live in `sgp/video_windows.h`. It still leaks into nominally generic surface storage: `sgp/vsurface.h` stores DirectDraw pointers in `SGPVSurface`. `sgp/WinFont.cpp` obtains a GDI device context from a private DirectDraw surface, making WinFont a special rendering migration item.
...
That portable foundation now exists. `presentation/DirtyRegionTracker` owns the characterized legacy invalidation, clipping, split, overflow-to-full, and reset rules and is used by the Windows refresh path. `presentation/PixelSurface` provides tested indexed8/RGB565 heap storage, aligned pitch, palettes, color keys, clipped/overlap-safe copying, fill, nearest-neighbor stretch, and fast unkeyed row copies. Its golden framebuffer test covers padded pitch, clipping, both color-key directions, cursor composition, overlap-safe horizontal and vertical movement, and backup/restore direction. `presentation/Presenter` describes final framebuffer upload, suspend, and resume without exposing SDL or Windows types. Every engine-created `SGPVSurface` and the reserved frame, back, and cursor buffers now use `PixelSurface` as canonical storage, regardless of legacy memory-placement flags. Frame refresh, tactical scrolling, overlays, fades, rain, cursor save/restore/composition, screenshot capture, and movie-frame capture stay on portable storage. The Windows `DirectDrawPresenter` owns the DirectDraw device, display mode, primary/flip surfaces, pixel-format query, palette attachment, completed-frame upload, windowed blit, fullscreen flip, retry, and presentation-surface recovery. The video manager now contains no DirectDraw API calls and no longer creates dedicated DirectDraw frame/cursor surfaces; its native accessors only bridge ordinary surface-manager compatibility mirrors needed by remaining mixed blits and Windows WinFont. The old post-flip primary-surface recovery copies are unnecessary because PixelSurface retains the complete frame. Compatibility mirrors and the reserved primary surface remain DirectDraw-owned for now.

`DIRECTDRAW_MIGRATION_NOTES.md` records the reserved and generic surface ownership graph, the creator/query references not visibly released by the legacy shutdown path, lost-surface and cursor rules, and the required replacement order. The backup/restore calls did pass source and destination opposite to their own documented intent; they now copy primary to backup on update and backup to primary on restore, with the direction locked down by the golden test.

WinFont policy is explicit: Windows retains the GDI-through-DirectDraw compatibility path, while portable builds force bitmap fonts. This keeps the common font interface portable without pretending the Chinese configuration's `USE_WINFONTS=1` has a native implementation yet.

### Input Migration Seam

`InputAtom`, engine event values, repeat/double-click policy, and the queue API in `sgp/input.h` remain the backend-neutral destination for host events. Gameplay cursor polling, clipping, warping, physical-key queries, and keyboard flushing now route through `platform/Input.h`; unused direct polls were deleted. The Windows adapter owns the thread-local mouse hook, Win32 mouse-message translation, async key polling, keyboard-message dispatch, and the exact screen/client conversions intercepted by cnc-ddraw. Common `input.cpp` consumes logical injected mouse events and no longer includes Windows or accesses the native window; Alt-Tab minimization uses a small platform window service. The portable backend provides deterministic cursor/restriction state until an SDL event source supplies real events.

## Audio and Cinematic Assessment

FMOD coupling is narrower than the rendering coupling. The library is linked only into the Windows SGP target, and direct calls are now confined to `sgp/audio/windows/FmodAudioBackend.cpp`. Most gameplay continues to use the existing `soundman.h`/`Sound Control` facade, while `soundman.cpp` retains cache/channel policy and depends only on the project-owned backend interface.
...
5. In progress: the application loop, cursor/input backend, exactly-once shutdown, dirty-region tracker, presenter contract, and heap-backed indexed/RGB565 surface primitives are portable and tested. Explicit system-memory generic surfaces and the reserved frame, back, and cursor buffers are PixelSurface-canonical. Tactical scrolling and cursor composition are portable, and DirectDraw now receives cadence-limited completed compositions at the Windows presentation boundary without blocking the game thread. The remaining presentation work is to convert default/video-memory generic surfaces, isolate fullscreen page recovery and primary upload behind the presenter contract, then replace that presenter with SDL.
