...
5. In progress: the application loop, cursor/input backend, exactly-once shutdown, dirty-region tracker, presenter contract, and heap-backed indexed/RGB565 surface primitives are portable and tested. The Windows host and input adapters retain current Wine/cnc-ddraw behavior. The remaining presentation step is to move reserved and generic logical surfaces together behind the portable storage/compositor boundary; converting them piecemeal would break mixed DirectDraw blits and WinFont's GDI escape hatch.
