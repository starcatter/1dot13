...
| Native core build | Root `ja2_shared_core` builds 22 first-party units with GCC and Clang; all 16 registered tests pass | Storage, resources, legacy file handles, compression, CPU line/image operations, UTF conversion, timing, sleep, and threading form a proven host-native island |
...
The project-owned `ja2::text` service now converts between UTF-8 byte strings and the engine's fixed-width `CHAR16` UTF-16 strings. It offers strict conversion with project-owned errors and explicit replacement conversion for compatibility boundaries. Characterization tests cover multilingual text, supplementary characters, embedded NULs, malformed UTF-8, lone UTF-16 surrogates, and third-party exception containment. utf8cpp was upgraded from 2.3.4 to the pinned 4.1.1 release and remains private to the adapter.

Lua wide strings and equipment-template filenames are the first migrated consumers. Lua conversions now preserve explicit Lua string lengths, and the Lua wide-string helpers no longer include Windows or depend on `wcslen`, `wcscpy`, or `wcscat`. Approximately 118 direct `MultiByteToWideChar`/`WideCharToMultiByte` call lines remain across 51 source files; localized XML, clipboard/input, multiplayer, and other gameplay boundaries should be migrated in characterized slices rather than by blind replacement.
...
- Completed: central UTF-8/UTF-16 conversion service with strict and replacement policies; call-site migration continues incrementally.
