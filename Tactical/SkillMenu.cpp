...
void
SkillMenuItem::SetupPopup(CHAR8* name)
{
	// create a popup
...
TraitSelection::Setup( UINT32 aVal )
{
	...
	for ( int i = 0; i < num; ++i)
	{
		...
		{
			pOption = new POPUP_OPTION(ja2::text::Utf16String(pStr), new popupCallbackFunction<void, UINT32>(&Wrapper_Setup_AutobandageSelection, traitarray[i]));
		}
		else
		{
			pOption = new POPUP_OPTION(ja2::text::Utf16String(pStr), new popupCallbackFunction<void, UINT32>(&Wrapper_Setup_SkillSelection, traitarray[i]));
		}
	...
	pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void,UINT32>( &Wrapper_Cancel_TraitSelection, 0 ) );
	GetPopup()->addOption( *pOption );
...
SkillSelection::Setup( UINT32 aVal )
{
	...
	if ( HAS_SKILL_TRAIT(pSoldier, aVal) )
	{
		...
		case RADIO_OPERATOR_NT:
			{
				for(UINT32 uiCounter = SKILLS_RADIO_FIRST; uiCounter <= SKILLS_RADIO_LAST; ++uiCounter)
				{
					...
					if ( uiCounter == SKILLS_RADIO_ARTILLERY)
						pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void,UINT32>( &Wrapper_Setup_ArtillerySector, uiCounter ) );
					else if ( uiCounter == SKILLS_RADIO_CALLREINFORCEMENTS)
						pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void,UINT32>( &Wrapper_Setup_ReinforcementSector, uiCounter ) );
					else
						pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void,UINT32>( &Wrapper_Function_SkillSelection, uiCounter ) );
		...
		case VARIOUSSKILLS:
			{
				for(UINT32 uiCounter = SKILLS_VARIOUS_FIRST; uiCounter <= SKILLS_VARIOUS_LAST; ++uiCounter)
				{
					...
					if ( uiCounter == SKILLS_DRAG )
						pOption = new POPUP_OPTION( ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Setup_DragSelection, uiCounter ) );
					else
						pOption = new POPUP_OPTION( ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_SkillSelection, uiCounter ) );
		...
		// cancel option
		swprintf( pStr, pSkillMenuStrings[SKILLMENU_CANCEL] );
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void,UINT32>( &Wrapper_Cancel_SkillSelection, 0 ) );
		GetPopup()->addOption( *pOption );
	}
...
ArtillerySector::Setup( UINT32 aVal )
{
	...
	if ( pSoldier->CanUseSkill(SKILLS_RADIO_ARTILLERY) )
	{
		...
		for (UINT8 i = 0; i < 4; ++i)
		{
			...
			swprintf( pStr, JA2_TEXT("%c%d"), loopY + 'A' - 1, loopX );

			pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Setup_ArtilleryTeam, sectornr ) );
		...
		// cancel option
		swprintf( pStr, pSkillMenuStrings[SKILLMENU_CANCEL] );
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void,UINT32>( &Wrapper_Cancel_ArtillerySector, 0 ) );
		GetPopup()->addOption( *pOption );
...
ArtilleryTeam::Setup( UINT32 aVal )
{
	...
	{		
		...
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_ArtilleryTeam, MILITIA_TEAM ) );
		...
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, INT8>( &Wrapper_Function_ArtilleryTeam, pSoldier->bTeam ) );
		...
		// cancel option
		swprintf( pStr, pSkillMenuStrings[SKILLMENU_CANCEL] );
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void,UINT32>( &Wrapper_Cancel_ArtilleryTeam, 0 ) );
		GetPopup()->addOption( *pOption );
...
ReinforcementSector::Setup( UINT32 aVal )
{
	...
	if ( pSoldier->CanUseSkill(aVal) )
	{
		...
		for (UINT8 i = 0; i < 4; ++i)
		{
			...
			pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Setup_ReinforcementNumber, sectornr ) );
		...
		// cancel option
		swprintf( pStr, pSkillMenuStrings[SKILLMENU_CANCEL] );
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Cancel_ReinforcementSector, 0 ) );
		GetPopup()->addOption( *pOption );
	}
...
ReinforcementNumber::Setup( UINT32 aVal )
{
	...
	if ( pSoldier->CanUseSkill(SKILLS_RADIO_CALLREINFORCEMENTS) )
	{
		...
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_ReinforcementNumber, 5 ) );
		...
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_ReinforcementNumber, 10 ) );
		...
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_ReinforcementNumber, 15 ) );
		...
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_ReinforcementNumber, 20 ) );
		...
		pOption = new POPUP_OPTION( ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_ReinforcementNumber, 30 ) );
		...
		pOption = new POPUP_OPTION( ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_ReinforcementNumber, 40 ) );
		...
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_ReinforcementNumber, numberofmilitia ) );
		GetPopup()->addOption( *pOption );

		// cancel option
		swprintf( pStr, pSkillMenuStrings[SKILLMENU_CANCEL] );
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Cancel_ReinforcementNumber, 0 ) );
		GetPopup()->addOption( *pOption );
	}
	...
	if ( pSoldier->CanUseSkill(aVal) )
	{
		...
		for ( SoldierID id = gTacticalStatus.Team[ OUR_TEAM ].bFirstID ; id <= gTacticalStatus.Team[ CIV_TEAM ].bLastID ; ++id )
		{
			...
			if ( iRange < 100 )
			{
				if ( id != pSoldier->ubID )
				{
					swprintf( pStr, JA2_TEXT("%s"), id->GetName() );

					pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT16>( &Wrapper_Function_SoldierSelection, id ) );
		...
		// cancel option
		swprintf( pStr, pSkillMenuStrings[SKILLMENU_CANCEL] );
		pOption = new POPUP_OPTION(ja2::text::Utf16String( pStr ), new popupCallbackFunction<void,UINT32>( &Wrapper_Cancel_SoldierSelection, 0 ) );
		GetPopup()->addOption( *pOption );
	}
...
DragSelection::Setup( UINT32 aVal )
{
	...
	if ( pSoldier->CanUseSkill( aVal ) )
	{
		...
		for ( SoldierID cnt = gTacticalStatus.Team[OUR_TEAM].bFirstID; cnt <= gTacticalStatus.Team[CIV_TEAM].bLastID; ++cnt )
		{
			...
			{
				swprintf( pStr, JA2_TEXT("%s"), cnt->GetName( ) );

				pOption = new POPUP_OPTION( ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_DragSelection, cnt ) );
		...
		for ( std::vector<INT16>::iterator it = corpseids.begin(); it != corpseids.end(); ++it )
		{
			...
			pOption = new POPUP_OPTION( ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_DragSelection, (*it) + NOBODY ) );
		...
		for ( int ubDirection = 0; ubDirection < NUM_WORLD_DIRECTIONS; ++ubDirection )
		{
				...
				&& IsDragStructurePresent( sTempGridNo, pSoldier->pathing.bLevel, tiletype, structurenumber, hitpoints, decalflag ) )
			{
				...
				if ( xmlentry >= 0 )
				{
					...
					swprintf(pStr, JA2_TEXT("%s (%s)"), buf, FaceDirs[gOneCDirection[ubDirection]]);

					// we have to use an offset of NOBODY in order to differentiate between person and corpse
					pOption = new POPUP_OPTION( ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_DragSelection_GridNo, sTempGridNo ) );
		...
		// cancel option
		swprintf( pStr, pSkillMenuStrings[SKILLMENU_CANCEL] );
		pOption = new POPUP_OPTION( ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Cancel_DragSelection, 0 ) );
		GetPopup( )->addOption( *pOption );
	}
...
EquipmentSelection::Setup(UINT32 aVal)
{
	...
	for ( UINT32 i = gEquipmentSelectionFirstEntryShown; i < gEquipmentSelectionLastEntryShown; ++i )
	{
		...
		pOption = new POPUP_OPTION( ja2::text::Utf16String( tmpchar ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_EquipmentSelection, i ) );
	...
	if ( gEquipmentSelectionMaxEntriesShown < gTemplateVector.size( ) )
	{
		...
		pOption = new POPUP_OPTION( ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_EquipmentSelection, 99999 ) );
		GetPopup( )->addOption( *pOption );
	}
	
	// cancel option
	swprintf(pStr, pSkillMenuStrings[SKILLMENU_CANCEL]);
	pOption = new POPUP_OPTION( ja2::text::Utf16String( pStr ), new popupCallbackFunction<void, UINT32>( &Wrapper_Function_EquipmentSelection, 99998 ) );
	GetPopup()->addOption(*pOption);
