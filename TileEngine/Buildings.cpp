...
BUILDING * GenerateBuilding( INT32 sDesiredSpot )
{
	...
	BUILDING* pBuilding = CreateNewBuilding(&ubBuildingID);
	if (!pBuilding)
	{
		return(NULL);
	}

	// RoofReachableTest() is called in the original codepath so this if conditional is commented out for now.
	// TODO: Convert original pathfinding code to use the same climbing spot flag as A* so we can get rid of the whole else clause
	//if (gGameSettings.fOptions[TOPTION_ALT_PATHFINDING])
	//{
	...
	if (gGameSettings.fOptions[TOPTION_ALT_PATHFINDING])
	{
		for (INT32 sGridNo = 0; sGridNo < WORLD_MAX; sGridNo++)
		{
				...
				gpWorldLevelData[sGridNo].ubExtFlags[1] & MAPELEMENT_EXT_CLIMBPOINT)
			{
				...
				if (fClimbUp)
				{
					for (UINT8 ubTestDir = 0; ubTestDir < NUM_WORLD_DIRECTIONS; ubTestDir += 2)
					{
