...
INT32 MineAMine( INT8 bMineIndex )
{
	...
	if( PlayerControlsMine(bMineIndex) )
	{
		...
		{
			// debug message
//			ScreenMsg( MSG_FONT_RED, MSG_DEBUG, JA2_TEXT("%s - Mine income from %s = $%d"), WORLDTIMESTR, pTownNames[ GetTownAssociatedWithMine( bMineIndex ) ], iAmtExtracted );
...
void IssueHeadMinerQuote( INT8 bMineIndex, UINT8 ubQuoteType )
{
	...
	{
		// debug message
		ScreenMsg( MSG_FONT_RED, MSG_DEBUG, JA2_TEXT("Head Miner #%s can't talk (quote #%d)"), gMercProfiles[ usHeadMinerProfileId ].zNickname, ubQuoteType);
		return;
	}
