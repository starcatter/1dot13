...
WRAPPED_STRING *LineWrap(INT32 iFont, UINT16 usLineWidthPixels, UINT16 *pusLineWidthIfWordIsWiderThenWidth, STR16 pString, ...)
{
	...
	CHAR16					TempString[1024];
	CHAR16		 pNullString[2];
	INT16					usCurIndex, usEndIndex, usDestIndex, usLastMaxWidthIndex;
	...
	*pusLineWidthIfWordIsWiderThenWidth = usLineWidthPixels;
	...
	if(DestString[usDestIndex]==NEWLINE_CHAR)
		...
		if ( WFStringPixLength( pCurrentStringLoc, iFont) < usLineWidthPixels )
		{
			...
			else
			{
				...
				//error
				usLineWidthPixels = 1 + WFStringPixLength( &TempString[usCurIndex], iFont);
...
//
UINT16 DisplayWrappedString(UINT16 usPosX, UINT16 usPosY, UINT16 usWidth, UINT8 ubGap, INT32 iFont, UINT8 ubColor, STR16 pString, UINT8 ubBackGroundColor, BOOLEAN fDirty, UINT32 uiFlags)
{
	...
	{
		pFirstWrappedString = LineWrapForSingleCharWords(iFont, usWidth, &usLineWidthIfWordIsWiderThenWidth,	pString);
	}
	else
	{
		pFirstWrappedString = LineWrap(iFont, usWidth, &usLineWidthIfWordIsWiderThenWidth,	pString);
	}
	...
	{
		DrawTextToScreen(pFirstWrappedString->sString, usPosX, usPosY, usWidth, iFont, ubColor, ubBackGroundColor, fDirty, uiFlags );
	...
	return(uiCounter * (WFGetFontHeight(iFont) + ubGap));
}
...
BOOLEAN DrawTextToScreen(const CHAR16* pStr, UINT16 usLocX, UINT16 usLocY, UINT16 usWidth, INT32 iFont, UINT8 ubColor, UINT8 ubBackGroundColor, BOOLEAN fDirty, UINT32 ulFlags)
{
	...
	{
		VarFindFontCenterCoordinates( usLocX, usLocY, usWidth, WFGetFontHeight(iFont), iFont, (INT16 *)&usPosX, (INT16 *)&usPosY, pStr );
	}
	else if( ulFlags & RIGHT_JUSTIFIED )
	{
		VarFindFontRightCoordinates( usLocX, usLocY, usWidth, WFGetFontHeight(iFont), iFont, (INT16 *)&usPosX, (INT16 *)&usPosY, pStr );
	}
	...
	if( ulFlags & TEXT_SHADOWED )
		ShadowText( FRAME_BUFFER, pStr, iFont, (UINT16)(usPosX-1), (UINT16)(usPosY-1 ) );
	...
	{
		usFontHeight= WFGetFontHeight( iFont );
		usStringWidth = WFStringPixLength( pStr, iFont );
...
if(g_lang != i18n::Lang::zh) {
	...
	return(usLinesUsed * ( WFGetFontHeight(iFont) + ubGap ) ); // +ubGap
}
															...
															UINT8 ubBackGroundColor, BOOLEAN fDirty, UINT32 uiFlags, BOOLEAN *fOnLastPageFlag)
{
 ...
 do
 {
	...
	*fOnLastPageFlag = FALSE;
	...
	// return how many Y pixels we used
	return(usLinesUsed * ( WFGetFontHeight(iFont) + ( UINT16 )ubGap )	); // +ubGap
}
...
BOOLEAN WillThisStringGetCutOff( INT32 iTotalYPosition, INT32 iBottomOfPage, INT32 iWrapWidth, INT32 iFont, STR16 pString, INT32 iGap, INT32 iPage )
{
...
// for file viewer
FileStringPtr GetFirstStringOnThisPage( FileStringPtr RecordList, INT32 iFont, UINT16 usWidth, UINT8 ubGap, INT32 iPage, INT32 iPageSize, FileRecordWidthPtr WidthList )
{
	// get the first record on this page - build pages up until this point
